...
const getLikedVideos = asyncHandler(async (req, res) => {
  ...
  const videos = likedVideos[0]?.video || [];
  return res
    .status(200)
    .json(new ApiResponse(20, videos, "videos fetched successfully"));
});
...
const toggleLike = asyncHandler(async (req, res) => {
  ...
  else throw new ApiError(400, "Invalid query string for toggle like");
...
const toggleCommentLike = asyncHandler(async (req, res) => {
  ...
  console.log(`Received toggleLike: ${toggleLike}`);
  ...
  const comment = await Comment.findById(commentId);
  if (!comment) throw new ApiError(400, "No any comment found");
...
const toggleTweetLike = asyncHandler(async (req, res) => {
  ...
  const tweet = await Tweet.findById(tweetId);
  if (!tweet) throw new ApiError(400, "No any tweet found here");
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { isLiked, totalLikes: totalLikes.length },
        "Tweet's like toggled successfully"
      )
    );
});

export {toggleLike, toggleVideoLike, toggleCommentLike, toggleTweetLike, getLikedVideos };
